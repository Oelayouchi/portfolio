...
export async function POST(request) {
  try {
    ...
    if (String(subject).length > 160 || String(description).length > 4000) {
      return Response.json({ error: 'Le message est trop long.' }, { status: 400 });
    }

    if (!process.env.RESEND_API_KEY) {
      return Response.json(
        { error: 'Le service d’envoi d’e-mails n’est pas encore configuré.' },
        { status: 503 },
      );
    }
      ...
      body: JSON.stringify({
        ...
        html: `
          <div style="font-family:Arial,sans-serif;line-height:1.6;color:#102033">
            ...
            <div style="padding:16px;border-radius:10px;background:#f4f8ff;border:1px solid #d7e7f7">
              ${safeDescription}
            </div>
          </div>
        `,
