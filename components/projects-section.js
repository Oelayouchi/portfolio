...
export default function ProjectsSection(){
  ...
  const visibleProjects=projects.slice(0,projectSlugs.length).map(p=>localizeProject(p,language));
  return <section id="projects" className="section shell projectShowcaseSection">
    <div className="projectShowcaseHeading"><span className="eyebrow">PORTFOLIO</span><h2>{t('navProjects').toUpperCase()}</h2><p>{intro[language]}</p></div>
    <div className="projectShowcaseGrid" aria-label={t('navProjects')}>
      {visibleProjects.map((project,index)=><ProjectShowcaseCard key={`${project.title}-${index}`} project={project} slug={projectSlugs[index]}/>) }
    </div>
  </section>;
}
