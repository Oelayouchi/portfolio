...
function Icon({ name, className = '' }) {
  const common = { width: 24, height: 24, viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: 2, strokeLinecap: 'round', strokeLinejoin: 'round', className, 'aria-hidden': 'true' };
...
export function Navigation() { return <header className="navBar"><nav className="nav shell" aria-label="Navigation principale"><a className="brand" href="#top" aria-label="Retour en haut">OE<span>.</span></a><div className="navRight"><div className="navlinks"><a className="active" href="#about">À propos</a><a href="#experience">Expérience</a><a href="#projects">Projets</a><a href="#data">Data</a><a href="#certifications">Certifications</a><a href="#contact">Contact</a></div><div className="themeToggle" aria-label="Sélecteur de thème visuel"><span><Icon name="moon" /></span><span className="themeActive"><Icon name="sun" /></span></div></div></nav></header>; }

export function HeroSection() {
  const axes = [{ number:'01',title:'Systèmes embarqués',detail:'C / C++ / Python / Hardware',icon:'chip',tone:'blue'},{number:'02',title:'Safety automobile',detail:'ISO 26262 / AUTOSAR',icon:'shield',tone:'cyan'},{number:'03',title:'Safety ferroviaire',detail:'EN 50126/28/29',icon:'train',tone:'green'},{number:'04',title:'Data Analytics',detail:'SQL / Power BI / Python',icon:'chart',tone:'purple'}];
  return <section id="top" className="heroSection"><div className="hero shell"><div className="heroIntro"><div className="heroBadge"><span className="heroBadgeDot"/>INGÉNIEUR & DATA EN RECONVERSION</div><h1>Oussama<br/><span>EL AYOUCHI</span></h1><p className="lead">Ingénieur systèmes embarqués & Safety,<br className="desktopBreak"/> actuellement en reconversion vers la <strong>Data Analyst.</strong></p><div className="heroActions"><a className="button primary" href="/documents/CV_Oussama_EL_AYOUCHI.pdf" target="_blank" rel="noreferrer"><Icon name="download"/>Télécharger mon CV</a><a className="button secondary" href="#contact"><Icon name="mail"/>Me contacter</a></div></div><aside className="heroCard"><h2>Mon parcours en 4 axes</h2><div className="axisList">{axes.map(axis=><div className={`axis axis-${axis.tone}`} key={axis.number}><div className="axisNumber">{axis.number}</div><div className="axisIcon"><Icon name={axis.icon}/></div><div className="axisContent"><h3>{axis.title}</h3><p>{axis.detail}</p></div><div className="axisArrow"><Icon name="chevron"/></div></div>)}</div></aside><a className="scrollHint" href="#about"><span className="mouseShape"><i/></span><span>Scroller pour en savoir plus</span><b>↓</b></a></div></section>;
}
