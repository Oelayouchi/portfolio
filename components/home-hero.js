...
const career = [
  { href: '#experience-parcelhome', period: '2022 – 2023', company: 'ParcelHome', role: 'Développeur électronique & logiciel', stack: 'C/C++ · Python · STM32' },
  { href: '#experience-continental', period: '2023 – 2024', company: 'Continental', role: 'Ingénieur sécurité logicielle automobile', stack: 'AUTOSAR · DOORS · ISO 26262' },
  { href: '#experience-alstom', period: '2024 – 2025', company: 'Alstom', role: 'Ingénieur sécurité ferroviaire', stack: 'FPGA · EN 50129' },
  { href: '#contact', period: '', company: 'À la recherche', role: 'Nouvelles opportunités Data & systèmes', stack: 'Data Analyst · Ingénierie · Alternance / CDI', search: true },
];
      ...
      <div className="shell homeHeroShell">
        <div className="homeHeroMain">
          <div className="homeIdentityCard">
            ...
            <div className="homeIdentityCopy"><strong>Oussama EL AYOUCHI</strong><span>Ingénieur systèmes embarqués & Safety</span></div>
        ...
        <div className="homeTools" aria-label="Technologies principales">
          {tools.map((tool) => <div className="homeTool" key={tool.name} title={tool.name}><span className="homeToolIcon" aria-hidden="true">{tool.mark}</span><strong>{tool.name}</strong></div>)}
        </div>
        <div className="homeCareer">
          ...
          <div className="homeCareerLine">
            {career.map((item) => <a className={`homeCareerItem${item.search ? ' homeCareerSearch' : ''}`} href={item.href} key={item.company}><span className="homeCareerDot" />{item.period && <small>{item.period}</small>}<strong>{item.company}</strong><p>{item.role}</p><em>{item.stack}</em></a>)}
          </div>
        </div>
      </div>
    </section>
