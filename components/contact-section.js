  ...
  return (
    <section id="contact" className="section shell contact contactEnhanced">
      <div className="contactIntro">
        ...
        <p>
          Je recherche une alternance en Data Analyse et je reste ouvert aux échanges autour
          de la Data, du logiciel embarqué et de la sûreté de fonctionnement.
        </p>

        <div className="contactQuickActions">
          <a className="button primary contactPhoneButton" href="tel:+33626294404">
            <span aria-hidden="true">☎</span>
            +33 6 26 29 44 04
          </a>
          <a
            className="button contactLinkedinButton"
        ...
        <form className="contactForm" onSubmit={handleSubmit}>
          ...
          <label>
            ...
            <input
              type="text"
              name="subject"
              placeholder="Proposition d’alternance, échange professionnel…"
              maxLength={160}
              required
            />
          </label>
          ...
          <button className="button primary contactSubmitButton" type="submit" disabled={status === 'sending'}>
            {status === 'sending' ? 'Envoi en cours…' : 'Envoyer le message'}
            <span aria-hidden="true">↗</span>
          </button>

          {message && (
            <p className={`contactFormStatus ${status}`} role="status">
              {message}
            </p>
          )}
        </form>
